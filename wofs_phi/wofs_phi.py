...
from metpy.calc import mean_pressure_weighted as p_weight_mean
#import multiprocessing as mp

#from . import config as c
#from . import utilities
#from . import predictor_extractor as pex
#from . import ml_driver
#from . import multiprocessing_driver as md
...
class MLGenerator: 
    ...
    def generate(self):
        ...
        forecast_specs = ForecastSpecs.create_forecast_specs(self.ps_files, self.wofs_files,\
                                                             c.all_fields_file, c.all_methods_file, c.single_pt_file)
        ...
        if (c.generate_forecasts == True):
            ...
            ps = PS.preprocess_ps(fcst_grid, forecast_specs, self.ps_path, self.ps_files, wofs)
            ...
            if (c.save_ncdf == True):
                ...
                if (c.plot_forecasts == True):
                    ...
                    full_ncdf_outname = "%s/%s" %(self.nc_outdir, MLGenerator.get_ncdf_outname(forecast_specs))

                    #TODO: Probably will update this later; for now, save to same path as netcdf 
                    #png_path = "%s/%s" %(self.nc_outdir, self.mode)
                    
                    if forecast_specs.ps_lead_time_start < 10:
                        plot_wofs_phi_warning_mode(full_ncdf_outname, c.png_outdir_warning, \
                    ...
                    if (forecast_specs.start_valid[2:] == '30') or (forecast_specs.start_valid[2:] == '00'):
                        plot_wofs_phi_forecast_mode(full_ncdf_outname, c.png_outdir_forecast, \
                            forecast_specs.wofs_init_time_dt, forecast_specs.before_00z_date, forecast_specs.ps_init_time_dt, \
                            forecast_specs.start_valid_dt, forecast_specs.end_valid_dt,\
                            forecast_specs.forecast_window, self.train_types)
                    ...
                    #            
                    #    plot_wofs_phi_forecast_mode(full_ncdf_outname, c.png_outdir, \
                    #        forecast_specs.wofs_init_time_dt, forecast_specs.before_00z_date, forecast_specs.ps_init_time_dt, \
                    #        forecast_specs.start_valid_dt, forecast_specs.end_valid_dt,\
                    #        forecast_specs.forecast_window, self.train_types)
    ...
    @staticmethod
    def get_predictor_names():
        
        variables = np.genfromtxt("%s/rf_variables.txt" %(c.wofs_and_ps_var_dir), dtype='str')
    ...
    def get_full_npy_filename(fSpecs):
        ...
        if not (c.model_type == 'wofs_psv2_no_torp'):
    ...
    @staticmethod
    def get_obs_full_npy_filenames(fSpecs, haz_name, str_radius, buffer_str):
        ...
        use_fname_1d = "%s_reps1d_%s_v%s-%s_r%skm%s.npy" %(haz_name, fSpecs.before_00z_date,\
                        fSpecs.start_valid, fSpecs.end_valid, str_radius, buffer_str)

        use_fname_2d = "%s_reps2d_%s_v%s-%s_r%skm%s.npy" %(haz_name, fSpecs.before_00z_date,\
                        fSpecs.start_valid, fSpecs.end_valid, str_radius, buffer_str)
    ...
    @staticmethod
    def get_obs_dat_filename(fSpecs, haz_name, str_radius, buffer_str):
        ...
        if c.model_type == 'wofs_psv2_no_torp':
            use_fname = "%s_reps1d_%s_%s_%s_v%s-%s_r%skm%s.dat"\
            %(haz_name, fSpecs.before_00z_date, fSpecs.wofs_init_time,\
              fSpecs.ps_init_time, fSpecs.start_valid, fSpecs.end_valid,\
              str_radius, buffer_str)
        else:
            model_type = c.model_type[5:]
            use_fname = "%s_%s_reps1d_%s_%s_%s_v%s-%s_r%skm%s.dat"\
              %(model_type, haz_name, fSpecs.before_00z_date, fSpecs.wofs_init_time,\
                fSpecs.ps_init_time, fSpecs.start_valid, fSpecs.end_valid,\
                str_radius, buffer_str)
    ...
    def get_ncdf_outname(fSpecs):
        ...
        wofs_date = fSpecs.wofs_init_time_dt.strftime("%Y%m%d")


        nc_name = "wofs_PHI_%s_%s_%s_%s_%s.nc" %(fSpecs.wofs_init_time, \
            fSpecs.ps_init_time, wofs_date, fSpecs.start_valid, fSpecs.end_valid)


        return nc_name

    @staticmethod
    def get_sr_filename(model_type, hazard, startmin, length, train_type, radius, modeStr):
        ...
        if train_type == 'warnings':
            sr_map_fname = '%s_%s_%s-%smin_%s_sr_map.csv' %(model_type, train_type, startmin, endmin, hazard)
        else:
            sr_map_fname = '%s_%s_%skm_%s-%smin_%s_sr_map.csv' %(model_type, train_type, radius, startmin, endmin, hazard)
        return sr_map_fname
...
class MLPrediction:
        ...
        for train_type in train_types: 
            for hazard in c.final_hazards:
                ...
                for r in range(len(c.obs_radii_str)):
                    ...
                    #create the explainability grids
                    #mlp.set_ti_grids
    ...
    def set_sr_probs(self, specs_obj, train_type, radius, mode_str):
        ...
        if c.wofs_spinup_time > 50:
            startmin -= 30
        sr_map_fname = MLGenerator.get_sr_filename(c.model_type, self.hazard, startmin, self.time_window, train_type, \
                                                   radius, mode_str)
    ...
    def set_ti_grids(self):
        
        ti_df = self.get_ti_df()
        
        #add one to indices list because of 'Unnamed: 0' column at the start of dataframe that just holds the index position
        wofs_predictors_sum = abs(test.iloc[:, c.wofs_indices+1]).sum(axis=1)
        ps_predictors_sum = abs(test.iloc[:, c.ps_indices+1]).sum(axis=1)
        lat_lon_sum = abs(test.iloc[:, c.lat_lon_indices+1]).sum(axis=1)
        if c.include_torp_in_predictors:
            torp_predictors_sum = abs(test.iloc[:, c.torp_indices+1]).sum(axis=1)
        else:
            torp_predictors_sum = wofs_predictors_sum
            torp_predictors_sum[:] = 0
        
        all_predictors_sum = wofs_predictors_sum + ps_predictors_sum + torp_predictors_sum + lat_lon_sum
        
        wofs_contrib = wofs_predictors_sum/all_predictors_sum
        ps_contrib = wofs_predictors_sum/all_predictors_sum
        torp_contrib = torp_predictors_sum/all_predictors_sum
        lat_lon_contrib = lat_lon_sum/all_predictors_sum
        
        #### TODO: What do we want to do with these values? ####
        
        return
    
    def get_ti_df(self):
        
        predictions, bias, contributions = ti.predict(self.rf_model, self.predictor_arr)
        interp = contributions[:,:,1] #only care about 'yes' prediction class
        
        var_names = MLGenerator.get_predictor_names()
        ti_df = pd.DataFrame(data=interp, columns=var_names)
        
        ti_df['probs'] = self.probs1d
        ti_df['bias'] = bias[:,1]
    ...
    def get_rf_filename(specs_obj, haz_name, radius, mode_str, train_string):
        ...
        if c.wofs_spinup_time > 50:
            start_init -= 30
        length = specs_obj.forecast_window
        end_init = start_init + length

        if (mode_str == "warning"):
            start_init = 30
            end_init = start_init + length
            pkl_filename = "%s/%s_%s_trained_wofsphi_%s_%smin_window%s-%s_r%skm.pkl"\
                        %(c.rf_dir, c.model_type, train_string, haz_name, specs_obj.forecast_window, start_init, end_init, radius)
            ...
            if start_init >= 180:
                ...
                start_init = 30
            pkl_filename = "%s/%s_%s_trained_wofsphi_%s_%smin_window%s-%s_r%skm.pkl"\
                        %(c.rf_dir, c.model_type, train_string, haz_name, specs_obj.forecast_window, start_init,\
                        end_init, radius)
...
class FinalNCFile:
    ...
    def save_ncdf(self, nc_outdir = c.ncdf_save_dir):
        ...
        full_outname = "%s/%s" %(nc_outdir, self.outname)
...
class ReportGenerator:
    ...
    def __init__(self, date_before_00z, rep_start_dt, rep_end_dt,\
                    ...
                    rand_inds, sampled_grid_1d, hazard, radius, radius_str, target, buffer_str):
    ...
    def generate(cls, fcst_specs, fcst_grid):
        ...
        for hazard in c.final_hazards:
            for i in range(len(c.obs_radii_float)):
                radius = c.obs_radii_float[i]
                radius_str = c.final_str_obs_radii[i]
                ...
                rep = ReportGenerator(dateBefore00z, repsStartDT, repsEndDT, c.report_time_buffer, startValidDT, endValidDT, \
                        ...
                        zero_rand_inds_arr, zero_rand_inds_arr, hazard, radius, radius_str, c.report_target, buffer_str)
    ...
    def save_to_file(self, specs):
        ...
        full_npy_one_d_name, full_npy_two_d_name = MLGenerator.get_obs_full_npy_filenames(\
                specs, self.hazard, str(self.radius_str), self.buffer_str)

        dat_name = MLGenerator.get_obs_dat_filename(specs, self.hazard, str(self.radius_str),\
                                                    self.buffer_str)
        

        #Save to appropriate files
        try:
            #will not recreate full npy files that eric has already made since the
            #full obs grids dont change with the ps version or usage of torp
...
class MLTrainer:
    ...
    TOTAL_GRID_POINTS = GRID_SHAPE_X*GRID_SHAPE_Y
    
    def __init__(self, dates, forecast_length, wofs_lead_time, num_folds, haz, wofs_spinup_time, radius, train_type, buffer_str,\
                 n_jobs = 36, n_trees = 200, criterion = 'entropy', max_depth = 15, min_samples_leaf = 20, max_features = "sqrt"):
        ...
        self.radius = radius
        self.train_type = train_type
        self.model_save_dir = '/work/ryan.martz/wofs_phi_data/%s_train/models/%s' %(train_type, c.model_type)
        #self.validation_dir = '/work/ryan.martz/wofs_phi_data/%s_train/validation_fcsts/%s' %(train_type, c.model_type)
        self.validation_dir = '/work/ryan.martz/wofs_phi_data/%s_train/test_fcsts/%s' %(train_type, c.model_type)
        self.test_dir = '/work/ryan.martz/wofs_phi_data/%s_train/test_fcsts/%s' %(train_type, c.model_type)
        
        top_inits = copy.deepcopy(c.top_hour_inits)
        bottom_inits = copy.deepcopy(c.bottom_hour_inits)
        inits = copy.deepcopy(top_inits)
        inits.extend(bottom_inits)
        self.inits = inits
    ...
    def do_training(self):
        ...
        for k in range(self.num_folds):
            ...
            if c.train_mode == 'train':
                self.save_model(k)
                print('model saved: fold ', k+1)
            if (c.train_mode == 'train') or (c.train_mode == 'validate'):
                self.run_on_validation(k)
                print('validation done: fold ', k+1)
            #if (c.train_mode == 'train') or (c.train_mode == 'validate') or (c.train_mode == 'test'):
            #    self.save_test_predictions(k, plotting = True)
            #    print('testing done: fold ', k+1, ' complete')
        #if c.use_avg_sr_map:
        #    self.create_sr_maps_from_validation_fcsts()
    ...
    def save_model(self, fold):
        ...
        clf_save_dir, clf_filename = self.get_model_fname_dir(fold)
        if os.path.exists('%s/%s' %(clf_save_dir, clf_filename)):
            return
        ...
        for date in fold_train_dates:
            ...
            for init in self.inits:
                if (init in c.bottom_hour_inits) and (self.lead_time + self.forecast_length > 180):
                    continue
                init_dt = utilities.make_dt_from_str(date, init)
                if init_dt.hour < c.wofs_reset_hour:
                    real_init_dt = init_dt + timedelta(days=1)
                else:
                    real_init_dt = init_dt
                spinup_dt = real_init_dt + dt.timedelta(seconds = (self.wofs_spinup_time*60))
                start_dt = real_init_dt + dt.timedelta(seconds = (self.lead_time*60))
                end_dt = start_dt + dt.timedelta(seconds = self.forecast_length*60)
                arr = self.get_predictors_fname(init_dt, spinup_dt, start_dt, end_dt)
                if arr[0] == '':
                    continue
                #get predictors returns array length 1 if there are no filenames, returns length 2 if there are (length 2 being the
                #fcst_specs)
                predictor_fname = arr[0]
                fcst_specs = arr[1]
                obs_fname = self.get_events_fname(init_dt, start_dt, end_dt, fcst_specs)
                try:
                    if not os.path.isfile(obs_fname):
                        print(obs_fname + ' does not exist')
                        continue
                except:
                    #no obs
                    continue
                    
                #these two filenames dont really matter as they get deleted right away
                store_dir = '/work/ryan.martz/wofs_phi_data/training_files'
                forecast_file = '%s/%s_%s_trained_training_fdata_%s_%s-%smin_r%skm_fold%s.dat' %(store_dir,\
                                                                                                 c.model_type, self.train_type,\
                                                                                                 self.hazard, self.lead_time,\
                                                                                                 self.lead_time + self.forecast_length,\
                                                                                                 self.radius, fold)
                obs_file = '%s/%s_%s_trained_training_odata_%s_%s-%smin_r%skm_fold%s.dat' %(store_dir,\
                                                                                            c.model_type, self.train_type,\
                                                                                            self.hazard, self.lead_time,\
                                                                                            self.lead_time + self.forecast_length,\
                                                                                            self.radius, fold)

                os.system('cat %s >> %s' %(predictor_fname, forecast_file))
                os.system('cat %s >> %s' %(obs_fname, obs_file))
        
        training_x = self.read_binary(forecast_file) #already float32... or should be probably not though because reasons
        training_y = np.float32(self.read_obs_binary(obs_file))

        os.system('rm %s' %(forecast_file))
        os.system('rm %s' %(obs_file))
        
        clf = RandomForestClassifier(n_jobs = self.n_jobs, n_estimators = self.n_trees, criterion = self.criterion, max_depth =
                                     self.max_depth, min_samples_leaf = self.min_samples_leaf, max_features = self.max_features)
    ...
    def run_on_validation(self, fold):
        fold_val_dates = self.date_val_folds[fold]
        
        all_probs_save_dir, all_probs_fname, all_events_fname = self.get_all_val_probs_events_fname_dir(fold)
        clf_save_dir, clf_filename = self.get_model_fname_dir(fold)
        ...
        all_probs = []
        all_events = []
        model = pickle.load(open('%s/%s' %(clf_save_dir, clf_filename), 'rb'))
        for date in fold_val_dates:
            for init in self.inits:
                if (init in c.bottom_hour_inits) and (self.lead_time + self.forecast_length > 180):
                    continue
                init_dt = utilities.make_dt_from_str(date, init)
                ...
                probs_1d, obs = self.get_probs(model, date, real_init_dt, do_saving = True)
                if probs_1d == [] or obs == []:
                    continue
                all_probs.extend(probs_1d)
                all_events.extend(obs)
                
                probs_2d = np.array(probs_1d).reshape(MLTrainer.GRID_SHAPE_X,MLTrainer.GRID_SHAPE_Y)
                ...
                if c.plot_in_training:
                    plot_save_dir, plot_file_sr, plot_file_raw = self.get_plot_fnames_dir(init_date_str, init_time_str,\
                                                                                          start_time_str, end_time_str)
                    title_sr, title_raw = self.get_plot_titles(init_date_str, init_time_str, start_time_str, end_time_str)
    ...
    def create_sr_maps_from_validation_fcsts(self):
        self.save_val_probs_events()
        all_data_dir, all_probs_fname, all_events_fname = self.get_all_validation_fcst_events_fname()
        all_probs = np.load('%s/%s' %(all_data_dir, all_probs_fname))
        all_events = np.load('%s/%s' %(all_data_dir, all_events_fname))
        thresholds = np.round(np.arange(0,1.01,0.01), 2)
        srs = MLTrainer.get_srs(all_probs, all_events, thresholds, include_less_10 = True)
        df = pd.DataFrame({'raw_prob': thresholds, 'SR': srs})
        sr_map_dir, sr_map_fname = self.get_avg_sr_map_fname_dir()
        utilities.save_data(sr_map_dir, sr_map_fname, df, 'csv')
        return
    
    def get_probs(self, model, date, real_init_dt, do_saving = False):
        if (real_init_dt.minute == 30) and (self.lead_time + self.forecast_length > 180):
            return [], []
        if real_init_dt.hour < c.wofs_reset_hour:
            init_dt = real_init_dt - timedelta(days=1)
        else:
            init_dt = real_init_dt
        spinup_dt = real_init_dt + dt.timedelta(seconds = (self.wofs_spinup_time*60))
        start_dt = real_init_dt + dt.timedelta(seconds = (self.lead_time*60))
        end_dt = start_dt + dt.timedelta(seconds = self.forecast_length*60)
        arr = self.get_predictors_fname(init_dt, spinup_dt, start_dt, end_dt, full_npy = True)
        if arr[0] == '':
            return [], []
        #get predictors returns array length 1 if there are no filenames, returns length 2 if there are (length 2 being the fcst_specs)
        predictor_fname = arr[0]
        fcst_specs = arr[1]
        predictors = np.load(predictor_fname)
        clf_probs = model.predict_proba(predictors)[:,1]
        probs_2d = clf_probs.reshape(MLTrainer.GRID_SHAPE_X, MLTrainer.GRID_SHAPE_Y)
        
        try:
            obs_fname = self.get_events_fname(init_dt, start_dt, end_dt, fcst_specs, full_npy = True)
            obs = np.load(obs_fname)
            events_len = len(obs)
            events = list(obs.reshape(events_len,))
            ...
            if do_saving:
                ...
                start_time_str = start_dt.strftime('%Y%m%d-%H%M').split('-')[1]
                end_time_str = end_dt.strftime('%Y%m%d-%H%M').split('-')[1]
                save_dir, save_file = self.get_val_probs_fnames_dir(init_date_str, init_time_str, start_time_str, end_time_str)
                utilities.save_data(save_dir, save_file, probs_2d, 'txt')
            
        except:
            #no obs file
            #try:
            #    rep_obj = ReportGenerator.generate(forecast_specs, fcst_grid)
            #    obs = np.load(obs_fname)
            #    events_len = len(obs)
            #    events = list(obs.reshape(events_len,))
            #except:
    ...
    def save_test_predictions(self, fold, plotting = False):
        ...
        if c.use_avg_sr_map:
            ...
            sr_map_dir, sr_map_fname = self.get_sr_map_fname_dir(fold)
        sr_map_df = pd.read_csv('%s/%s' %(sr_map_dir, sr_map_fname))
        all_srs = []
        all_events = []
        all_probs = []
        model = pickle.load(open('%s/%s' %(clf_save_dir, clf_filename), 'rb'))
        
        for date in fold_test_dates:
            for init in self.inits:
                if (init in c.bottom_hour_inits) and (self.lead_time + self.forecast_length > 180):
                    continue
                init_dt = utilities.make_dt_from_str(date, init)
                ...
                probs_1d, obs = self.get_probs(model, date, real_init_dt)
                if probs_1d == [] or obs == []:
                    #test for both probs and obs, shouldn't include obs in testing if they couldn't be found, unfair to model
                    continue
                probs_2d = np.array(probs_1d).reshape(MLTrainer.GRID_SHAPE_X,MLTrainer.GRID_SHAPE_Y)
                srs1d, srs2d = MLTrainer.probs_to_srs(probs_1d, sr_map_df)
                ...
                if c.plot_in_training:
                    plot_save_dir, plot_file_sr, plot_file_raw = self.get_plot_fnames_dir(init_date_str, init_time_str, start_time_str,
                                                                                          end_time_str)
                    title_sr, title_raw = self.get_plot_titles(init_date_str, init_time_str, start_time_str, end_time_str)
                ...
                save_dir, save_file_sr, save_file_raw, events_file = self.get_test_probs_fnames_dir(fold, init_date_str,\
                                                                                                    init_time_str, start_time_str,\
                                                                                                    end_time_str)
                utilities.save_data(save_dir, save_file_sr, srs2d, 'txt')
                utilities.save_data(save_dir, save_file_raw, probs_2d, 'txt')
                utilities.save_data(save_dir, events_file, obs, 'txt')
        
        self.save_test_probs_events_fold(fold_test_dates, fold)
    ...
    def get_predictors_fname(self, init_dt, spinup_dt, start_dt, end_dt, full_npy = False):
        ...
        ps_files, is_recent_ps_file = self.find_ps_files(spinup_dt)
        ...
        if full_npy == False:
            ...
            pred_filename = c.train_fcst_full_npy_dir + '/' + MLGenerator.get_full_npy_filename(fcst_specs)
    ...
    def get_events_fname(self, init_dt, start_dt, end_dt, fcst_specs, full_npy = False):
        ...
        init_str = init_dt.strftime('%H%M')
        start_str = start_dt.strftime('%H%M')
        end_str = end_dt.strftime('%H%M')
        if self.wofs_spinup_time % 2 == 1:
            wofs_spinup = self.wofs_spinup_time - 1
        else:
            wofs_spinup = self.wofs_spinup_time
        ps_time = init_dt + dt.timedelta(seconds = wofs_spinup*60)
        ps_time_str = ps_time.strftime('%H%M')
        
        full_obs_npy_fname = '%s/%s_reps1d_%s_v%s-%s_r%skm%s.npy' %(c.train_obs_full_npy_dir, self.hazard, day_str,\
                                                                  start_str, end_str, self.radius, self.buffer_str)
        
        if c.model_type == 'wofs_psv2_no_torp':
            sampled_obs_fname = '%s/%s_reps1d_%s_%s_%s_v%s-%s_r%skm%s.dat'\
            %(c.train_obs_dat_dir, self.hazard, day_str, init_str, ps_time_str, start_str,\
              end_str, self.radius, self.buffer_str)
        else:
            sampled_obs_fname = '%s/%s_%s_reps1d_%s_%s_%s_v%s-%s_r%skm%s.dat'\
            %(c.train_obs_dat_dir, c.model_type[5:], self.hazard, day_str, init_str,\
              ps_time_str, start_str, end_str, self.radius, self.buffer_str)
        
        warnings_npy_fname = 'length_%s/%s/%s_warnings_%s_v%s-%s_1d.npy'\
        %(self.forecast_length, self.hazard, self.hazard, day_str, start_str, end_str)
        
        obs_and_warnings_npy_fname = 'length_%s/%s/%s_obs_and_warnings_%s_v%s-%s_r%skm_1d.npy'\
        %(self.forecast_length, self.hazard, self.hazard, day_str, start_str, end_str, self.radius)
        
        if full_npy == True:
            full_obs_and_warnings_npy_fname = '%s/%s' %(c.train_obs_and_warnings_full_1d_npy_dir, obs_and_warnings_npy_fname)
            full_warnings_npy_fname = '%s/%s' %(c.train_warnings_full_1d_npy_dir, warnings_npy_fname)
            
            if os.path.exists(full_obs_npy_fname) and os.path.exists(full_warnings_npy_fname):#\
            #and os.path.exists(full_obs_and_warnings_npy_fname):
                if self.train_type == 'obs':
                    return full_obs_npy_fname
                elif self.train_type == 'warnings':
                    return full_warnings_npy_fname
                elif self.train_type == 'obs_and_warnings':
                    return full_obs_and_warnings_npy_fname
            else:
                return ''
        
        if c.model_type == 'wofs_psv2_no_torp':
            sampled_warnings_fname = 'length_%s/wofs_lead_%s/%s/sampled_%s_warnings_%s_%s_%s_v%s-%s.dat'\
            %(self.forecast_length, self.lead_time, self.hazard, self.hazard, day_str,\
              init_str, ps_time_str, start_str, end_str)
            
            sampled_obs_and_warnings_fname = 'length_%s/wofs_lead_%s/%s/sampled_%s_obs_and_warnings_%s_%s_%s_v%s-%s_r%skm.dat'\
            %(self.forecast_length, self.lead_time, self.hazard, self.hazard, day_str,\
              init_str, ps_time_str, start_str, end_str, self.radius)
        
        else:
            sampled_warnings_fname = 'length_%s/wofs_lead_%s/%s/%s_sampled_%s_warnings_%s_%s_%s_v%s-%s.dat'\
            %(self.forecast_length, self.lead_time, self.hazard, c.model_type[5:], self.hazard,\
              day_str, init_str,ps_time_str, start_str, end_str)
            
            sampled_obs_and_warnings_fname = 'length_%s/wofs_lead_%s/%s/%s_sampled_%s_obs_and_warnings_%s_%s_%s_v%s-%s_r%skm.dat'\
            %(self.forecast_length, self.lead_time, self.hazard, c.model_type[5:], self.hazard,\
              day_str, init_str,ps_time_str, start_str, end_str, self.radius)
            
        full_sampled_obs_and_warnings_fname = '%s/%s' %(c.train_obs_and_warnings_sampled_dat_dir,\
                                                        sampled_obs_and_warnings_fname)
        full_sampled_warnings_fname = '%s/%s' %(c.train_warnings_sampled_dat_dir, sampled_warnings_fname)
        
        if os.path.exists(sampled_obs_fname) and os.path.exists(full_sampled_warnings_fname):#\
        #and os.path.exists(full_sampled_obs_and_warnings_fname):
            if self.train_type == 'obs':
                return sampled_obs_fname
            elif self.train_type == 'warnings':
                return full_sampled_warnings_fname
            elif self.train_type == 'obs_and_warnings':
                return full_sampled_obs_and_warnings_fname
        else:
            return ''
    ...
    def set_chunk_splits(self):
        ...
        if subsets == 3:
            ...
            for i in range(self.num_folds):
                if i == 0:
                    self.date_train_folds.append(self.dates[chunk_splits[0]:chunk_splits[self.num_folds-1]])
                    self.date_val_folds.append(self.dates[chunk_splits[self.num_folds-1]:chunk_splits[self.num_folds]])
                else:
                    train_folds = self.dates[chunk_splits[i]:chunk_splits[self.num_folds]]
                    train_folds.extend(self.dates[chunk_splits[0]:chunk_splits[i-1]])
                    self.date_train_folds.append(train_folds)
                    self.date_val_folds.append(self.dates[chunk_splits[i-1]:chunk_splits[i]])
    ...
    def find_wofs_ALL_files(self, init_dt, start_dt, end_dt):
        ...
        init_date_dir_date_str = init_dt.strftime('%Y%m%d-%H%M').split('-')[0]
        init_time_str = init_dt.strftime('%Y%m%d-%H%M').split('-')[1]
        wofs_date_dir = c.wofs_dir + init_date_dir_date_str + '/' + init_time_str
        wofs_files = []
        if init_dt.hour < c.wofs_reset_hour:
            init_dt = init_dt + dt.timedelta(days = 1)
        init_date_str = init_dt.strftime('%Y%m%d-%H%M').split('-')[0]
        if c.use_ALL_files:
            for i in range(int((self.forecast_length/c.wofs_update_rate) + 1)):
                valid_dt = start_dt + dt.timedelta(seconds = ((i*c.wofs_update_rate)*60))
                valid_time_str = valid_dt.strftime('%Y%m%d-%H%M').split('-')[1]
                wofs_num = int(i+(self.lead_time/c.wofs_update_rate))
                if wofs_num < 10:
                    wofs_file = 'wofs_ALL_0%s_%s_%s_%s.nc' %(str(wofs_num), init_date_str, init_time_str, valid_time_str)
                else:
                    wofs_file = 'wofs_ALL_%s_%s_%s_%s.nc' %(str(wofs_num), init_date_str, init_time_str, valid_time_str)
                if os.path.isfile(wofs_file):
            ...
            for i in range(int((self.forecast_length/c.wofs_update_rate) + 1)):
                ...
                if wofs_num < 10:
                    ...
                    wofs_file = 'wofs_ALL_%s_%s_%s_%s.nc' %(str(wofs_num), init_date_str, init_time_str, valid_time_str)
                if os.path.isfile(wofs_date_dir + '/' + wofs_file) or (os.path.isfile(wofs_date_dir + '/' + wofs_file.replace('ALL', 'ENV')) and os.path.isfile(wofs_date_dir + '/' + wofs_file.replace('ALL', 'ENS')) and os.path.isfile(wofs_date_dir + '/' + wofs_file.replace('ALL', 'SVR'))):
                    wofs_files.append(wofs_file)
        
        return wofs_files, wofs_date_dir
    
    def find_ps_files(self, spinup_dt):
        ...
        if spinup_dt.minute % 2 == 1:
            spinup_dt -= dt.timedelta(seconds = 60)
        ps_dts = ml_driver.MLDriver.get_ps_datetimes(spinup_dt)
        ps_files = ml_driver.MLDriver.get_ps_names_from_dt_list(ps_dts)
        recent_files = False
        curr_time = spinup_dt - dt.timedelta(seconds = c.ps_wofs_buffer*60)
        for ps_file in ps_files:
            time_str, date_str = ForecastSpecs.find_ps_date_time(ps_file, c.ps_version)
            ps_time = utilities.make_dt_from_str(date_str, time_str)
            if (curr_time - ps_time).seconds/60 <= 10:
                recent_files = True
            
        #earliest_time = spinup_dt - dt.timedelta(hours = 3) - dt.timedelta(seconds = c.ps_wofs_buffer*60)
        ...
        #    if os.path.isfile(c.ps_dir + '/' + ps_file):
        #        ps_files.append(ps_file)
        #        lead = (spinup_dt - curr_time).seconds*60
        #        if lead >= (c.ps_search_minutes - c.ps_recent_file_threshold):
        #            recent_files = True
        #   curr_time = curr_time - dt.timedelta(seconds = 60)
    ...
    def save_val_probs_events(self):
        all_probs = []
        all_events = []
        for date in self.dates:
            for init in self.inits:
                ...
                fcst_path, fcst_file, events_path, events_file = self.get_validation_fcst_events_fname(date, init, start_str, end_str)
                
                try:
                    #creation_time = dt.datetime.fromtimestamp(os.stat('%s/%s' %(fcst_path, fcst_file)).st_mtime)
                    #if creation_time.month < 10:
                    #    os.remove('%s/%s' %(fcst_path, fcst_file))
                    probs_2d = np.genfromtxt('%s/%s' %(fcst_path, fcst_file))
                    probs = list(probs_2d.reshape(MLTrainer.TOTAL_GRID_POINTS,))
                    events = np.load('%s/%s' %(events_path, events_file)).reshape(MLTrainer.TOTAL_GRID_POINTS,)
                    if not (events.shape == (MLTrainer.TOTAL_GRID_POINTS,)):
                        print(events.shape)
                        print(events_file)
                except:
                    continue
        ...
        save_dir, save_probs_fname, save_events_fname = self.get_all_validation_fcst_events_fname()
    ...
    def aggregate_test_predictions(self):
        ...
        for date in self.dates:
            for init in self.inits:
                ...
                fcst_path, probs_file, sr_file, events_path, events_file = self.get_test_fcst_events_fname(date, init, start_str, end_str)
                
                try:
                    probs_2d = np.genfromtxt('%s/%s' %(fcst_path, probs_file))
                    probs = probs_2d.reshape(MLTrainer.TOTAL_GRID_POINTS,)
                    srs_2d = np.genfromtxt('%s/%s' %(fcst_path, sr_file))
                    srs = srs_2d.reshape(MLTrainer.TOTAL_GRID_POINTS,)
                    events = np.load('%s/%s' %(events_path, events_file)).reshape(MLTrainer.TOTAL_GRID_POINTS,)
                    events.reshape(MLTrainer.TOTAL_GRID_POINTS,)
                    daily_sr_bs = brier_score_loss(events, srs)
                    daily_prob_bs = brier_score_loss(events, probs)
                ...
                all_srs.extend(srs)
                all_events.extend(events)
                all_daily_sr_bs.append(daily_sr_bs)
                all_daily_prob_bs.append(daily_prob_bs)
                all_daily_climo.append(climo)
        
        save_dir, save_probs_fname, save_srs_fname, save_events_fname,\
        save_daily_prob_bs_fname, save_daily_sr_bs_fname, save_daily_climo_fname =\
        self.get_all_test_fcst_events_fname()
        
        all_probs = np.array(all_probs)
        all_srs = np.array(all_srs)
        all_events = np.array(all_events)
        all_daily_sr_bs = np.array(all_daily_sr_bs)
        all_daily_prob_bs = np.array(all_daily_prob_bs)
        all_daily_climo = np.array(all_daily_climo)
        
        print(all_srs.shape, all_probs.shape, all_events.shape)
        utilities.save_data(save_dir, save_srs_fname, all_srs, 'npy')
        utilities.save_data(save_dir, save_probs_fname, all_probs, 'npy')
        utilities.save_data(save_dir, save_events_fname, all_events, 'npy')
    ...
    def save_test_probs_events_fold(self, dates, fold):
        all_srs = []
        all_probs = []
        all_events = []
        for date in dates:
        ...
        save_dir, save_probs_fname, save_srs_fname, save_events_fname = self.get_all_test_fcst_events_fname_fold(fold)
    ...
    def get_validation_fcst_events_fname(self, date, init, start_str, end_str):
        fcst_path = '/work/ryan.martz/wofs_phi_data/%s_train/validation_fcsts/%s/%s/wofslag_%s/length_%s/%s/%s' %(self.train_type, c.model_type,
                                                                                                                  self.hazard, self.wofs_spinup_time,
                                                                                                                  self.forecast_length, date, init)
        fcst_file = '%s_%s_trained_rf_%s_raw_probs_%s_init%s_v%s-%s_r%skm.txt' %(c.model_type, self.train_type, self.hazard, date, init,
                                                                                 start_str, end_str, self.radius)
        if self.train_type == 'obs':
            events_dir = '/work/eric.loken/wofs/2024_update/SFE2024/obs/full_npy'
            events_file = '%s_reps1d_%s_v%s-%s_r%skm%s.npy' %(self.hazard, date, start_str, end_str, self.radius, self.buffer_str)
        else:
            events_dir = '/work/ryan.martz/wofs_phi_data/training_data/%s/full_1d_%s/length_%s/%s' %(self.train_type,\
                                                                                                     self.train_type,\
                                                                                                     self.forecast_length,\
                                                                                                     self.hazard)
            events_file = '%s_%s_%s_v%s-%s_r%skm_1d.npy' %(self.hazard, self.train_type, date, start_str, end_str, self.radius)
        
        return fcst_path, fcst_file, events_dir, events_file
    
    def get_test_fcst_events_fname(self, date, init, start_str, end_str):
        fcst_path = '/work/ryan.martz/wofs_phi_data/%s_train/test_fcsts/%s/%s/wofslag_%s/length_%s/%s/%s' %(self.train_type, c.model_type,
                                                                                                                  self.hazard, self.wofs_spinup_time,
                                                                                                                  self.forecast_length, date, init)
        probs_file = '%s_%s_trained_rf_%s_raw_probs_%s_init%s_v%s-%s_r%skm.txt' %(c.model_type, self.train_type, self.hazard, date, init,
                                                                                 start_str, end_str, self.radius)
        
        if c.use_avg_sr_map:
            sr_file = '%s_%s_trained_rf_%s_avg_sr_probs_%s_init%s_v%s-%s_r%skm.txt' %(c.model_type, self.train_type, self.hazard, date, init, start_str,
                                                                                      end_str, self.radius)
        else:
            sr_file = '%s_%s_trained_rf_%s_sr_probs_%s_init%s_v%s-%s_r%skm.txt' %(c.model_type, self.train_type, self.hazard, date, init, start_str, end_str,
                                                                                  self.radius)
        ...
        if self.train_type == 'obs':
            ...
            events_dir = '/work/ryan.martz/wofs_phi_data/training_data/%s/full_1d_%s/length_%s/%s' %(self.train_type, self.train_type,\
                                                                                                     self.forecast_length, self.hazard)
            events_file = '%s_%s_%s_v%s-%s_r%skm_1d.npy' %(self.hazard, self.train_type, date, start_str, end_str, self.radius)
        
        return fcst_path, probs_file, sr_file, events_dir, events_file
    
    def get_predictor_path_file(self, date, init, start_str, end_str):
        
        pred_dir = c.train_fcst_full_npy_dir
        
        init_dt = dt.datetime(1970, 1, 1, int(init[0:2]), int(init[2:]))
        ps_init_dt = init_dt + dt.timedelta(seconds = self.wofs_spinup_time*60)
        if ps_init_dt.minute % 2 == 1:
            ps_init_dt = ps_init_dt - dt.timedelta(seconds = 60)
        ps_init = ps_init_dt.strftime("%H%M")
        
        if c.model_type == 'wofs_psv2_no_torp':
            pred_fname = 'wofs1d_%s_%s_%s_v%s-%s.npy' %(date, init, ps_init, start_str, end_str)
        else:
            pred_fname = 'wofs1d_%s_%s_%s_%s_v%s-%s.npy' %(c.model_type[5:], date, init, ps_init, start_str, end_str)
        
        return pred_dir, pred_fname
    
    def get_all_validation_fcst_events_fname(self):
        save_dir = '/work/ryan.martz/wofs_phi_data/%s_train/validation_fcsts/%s/%s/wofslag_%s/length_%s' %(self.train_type, c.model_type, self.hazard,
                                                                                                           self.wofs_spinup_time, self.forecast_length)
        save_probs_fname = '%s_%s_r%skm_trained_all_%s_raw_probs_%s-%smin.npy' %(c.model_type, self.train_type, self.radius, self.hazard,
                                                                                 self.lead_time, self.lead_time + self.forecast_length)
        save_events_fname = 'all_%s_%s_%s-%smin_r%skm.npy'\
        %(self.hazard, self.train_type, self.lead_time, self.lead_time + self.forecast_length, self.radius)
        
        return save_dir, save_probs_fname, save_events_fname
    
    def get_all_test_fcst_events_fname(self):
        save_dir = '/work/ryan.martz/wofs_phi_data/%s_train/test_fcsts/%s/%s/wofslag_%s/length_%s'\
        %(self.train_type, c.model_type, self.hazard, self.wofs_spinup_time, self.forecast_length)
        save_probs_fname = '%s_%s_r%skm_trained_all_%s_test_raw_probs_%s-%smin.npy'\
        %(c.model_type, self.train_type, self.radius, self.hazard, self.lead_time,\
          self.lead_time + self.forecast_length)
        save_daily_prob_bs_fname = '%s_%s_r%skm_trained_all_%s_test_daily_prob_bs_%s-%smin.npy'\
        %(c.model_type, self.train_type, self.radius, self.hazard, self.lead_time,\
          self.lead_time + self.forecast_length)
        if c.use_avg_sr_map:
            save_srs_fname = '%s_%s_r%skm_trained_all_%s_avg_sr_probs_%s-%smin.npy'\
            %(c.model_type, self.train_type, self.radius, self.hazard, self.lead_time,\
              self.lead_time + self.forecast_length)
            ...
            save_srs_fname = '%s_%s_r%skm_trained_all_%s_sr_probs_%s-%smin.npy'\
            %(c.model_type, self.train_type, self.radius, self.hazard, self.lead_time,\
              self.lead_time + self.forecast_length)
        save_events_fname = 'all_%s_%s_%s-%smin_r%skm.npy'\
        %(self.hazard, self.train_type, self.lead_time, self.lead_time + self.forecast_length,\
          self.radius)
        save_daily_climo_fname = 'all_%s_%s_%s-%smin_r%skm_daily_climos.npy'\
        %(self.hazard, self.train_type, self.lead_time, self.lead_time + self.forecast_length,\
          self.radius)
        
        return save_dir, save_probs_fname, save_srs_fname, save_events_fname,\
    save_daily_prob_bs_fname, save_daily_sr_bs_fname, save_daily_climo_fname
    
    def get_all_test_fcst_events_fname_fold(self, fold):
        save_dir = '/work/ryan.martz/wofs_phi_data/%s_train/test_fcsts/%s/%s/wofslag_%s/length_%s/fold%s' %(self.train_type, c.model_type,
                                                                                                            self.hazard, self.wofs_spinup_time,
                                                                                                            self.forecast_length, fold)
        save_probs_fname = '%s_%s_r%skm_trained_all_%s_test_raw_probs_%s-%smin_fold%s.npy' %(c.model_type, self.train_type, self.radius, self.hazard,
                                                                                             self.lead_time, self.lead_time + self.forecast_length, fold)
        if c.use_avg_sr_map:
            save_srs_fname = '%s_%s_r%skm_trained_all_%s_avg_sr_probs_%s-%smin_fold%s.npy' %(c.model_type, self.train_type, self.radius, self.hazard,
                                                                                            self.lead_time, self.lead_time + self.forecast_length, fold)
        else:
            save_srs_fname = '%s_%s_r%skm_trained_all_%s_sr_probs_%s-%smin_fold%s.npy' %(c.model_type, self.train_type, self.radius, self.hazard,
                                                                                        self.lead_time, self.lead_time + self.forecast_length, fold)
        save_events_fname = 'all_%s_%s_%s-%smin_r%skm_fold%s.npy' %(self.hazard, self.train_type, self.lead_time, self.lead_time + self.forecast_length,
                                                                    self.radius, fold)
        
        return save_dir, save_probs_fname, save_srs_fname, save_events_fname
        
    
    def get_model_fname_dir(self, fold):
        if self.train_type == 'warnings':
            save_dir = '%s/%s/wofslag_%s/length_%s' %(self.model_save_dir, self.hazard, self.wofs_spinup_time, self.forecast_length)
            save_file = '%s_%s_trained_wofsphi_%s_%smin_window%s-%s_fold%s.pkl' %(c.model_type, self.train_type, self.hazard, self.forecast_length,
                                                                                  self.lead_time, self.lead_time + self.forecast_length, fold)
        else:
            save_dir = '%s/%s/wofslag_%s/length_%s' %(self.model_save_dir, self.hazard, self.wofs_spinup_time, self.forecast_length)
            save_file = '%s_%s_trained_wofsphi_%s_%smin_window%s-%s_r%skm_fold%s.pkl' %(c.model_type, self.train_type, self.hazard,
                                                                                        self.forecast_length, self.lead_time, self.lead_time +
                                                                                        self.forecast_length, self.radius, fold)
        
        return save_dir, save_file
    
    def get_all_val_probs_events_fname_dir(self, fold):
        start_min = self.lead_time
        end_min = self.lead_time + self.forecast_length
        all_probs_save_dir = '%s/%s/wofslag_%s/length_%s/all_raw_probs_fold%s' %(self.validation_dir, self.hazard, self.wofs_spinup_time,
                                                                                 self.forecast_length, fold)
        if self.train_type == 'warnings':
            all_probs_fname = '%s_%s_trained_all_rf_%s_raw_probs_spinup%smin_length%s_min%s-%s_fold%s.npy' %(c.model_type, self.train_type, self.hazard,
                                                                                                          self.wofs_spinup_time, self.forecast_length,
                                                                                                          start_min,  end_min, fold)
            all_events_fname = '%s_%s_trained_all_%s_events_spinup%smin_length%s_min%s-%s_fold%s.npy' %(c.model_type, self.train_type, self.hazard,
                                                                                                     self.wofs_spinup_time,self.forecast_length,
                                                                                                     start_min, end_min, fold)
        else:
            all_probs_fname = '%s_%s_trained_all_rf_%s_raw_probs_spinup%smin_length%s_min%s-%s_r%skm_fold%s.npy' %(c.model_type, self.train_type, self.hazard,
                                                                                                                self.wofs_spinup_time,
                                                                                                                self.forecast_length, start_min,
                                                                                                                end_min, self.radius, fold)
            all_events_fname = '%s_%s_trained_all_%s_events_spinup%smin_length%s_min%s-%s_r%skm_fold%s.npy' %(c.model_type, self.train_type, self.hazard,
                                                                                                           self.wofs_spinup_time, self.forecast_length,
                                                                                                           start_min, end_min, self.radius, fold)
        return all_probs_save_dir, all_probs_fname, all_events_fname
    
    def get_val_probs_fnames_dir(self, init_date_str, init_time_str, start_time_str, end_time_str):
        save_dir = '%s/%s/wofslag_%s/length_%s/%s/%s' %(self.validation_dir, self.hazard, self.wofs_spinup_time, self.forecast_length, init_date_str,
                                                        init_time_str)
        if self.train_type == 'warnings':
            save_file = '%s_%s_trained_rf_%s_raw_probs_%s_init%s_v%s-%s.txt' %(c.model_type, self.train_type, self.hazard, init_date_str,
                                                                               init_time_str, start_time_str, end_time_str)
        else:
            save_file = '%s_%s_trained_rf_%s_raw_probs_%s_init%s_v%s-%s_r%skm.txt' %(c.model_type, self.train_type, self.hazard, init_date_str,
                                                                                     init_time_str, start_time_str, end_time_str, self.radius)
    ...
    def get_all_test_sr_events_fname_dir(self, fold):
        start_min = self.lead_time
        end_min = self.lead_time + self.forecast_length
        save_dir = '%s/%s/wofslag_%s/length_%s/fold%s' %(self.test_dir, self.hazard, self.wofs_spinup_time, self.forecast_length, fold)
        if self.train_type == 'warnings':
            if c.use_avg_sr_map:
                all_srs_fname = '%s_%s_trained_all_rf_%s_avg_sr_probs_spinup%smin_length%smin_%s-%s_fold%s.npy' %(c.model_type, self.train_type, self.hazard,
                                                                                                           self.wofs_spinup_time, self.forecast_length,
                                                                                                           start_min, end_min, fold)
            else:
                all_srs_fname = '%s_%s_trained_all_rf_%s_sr_probs_spinup%smin_length%smin_%s-%s_fold%s.npy' %(c.model_type, self.train_type, self.hazard,
                                                                                                           self.wofs_spinup_time, self.forecast_length,
                                                                                                           start_min, end_min, fold)
            all_probs_fname = '%s_%s_trained_all_rf_%s_raw_probs_spinup%smin_length%smin_%s-%s_fold%s.npy' %(c.model_type, self.train_type, self.hazard,
                                                                                                       self.wofs_spinup_time, self.forecast_length,
                                                                                                       start_min, end_min, fold)
            all_events_fname = '%s_%s_trained_all_%s_events_spinup%smin_length%smin_%s-%s_fold%s.npy' %(c.model_type, self.train_type, self.hazard,
                                                                                                     self.wofs_spinup_time, self.forecast_length,
                                                                                                     start_min, end_min, fold)
        else:
            if c.use_avg_sr_map:
                all_srs_fname = '%s_%s_trained_all_rf_%s_avg_sr_probs_spinup%smin_length%smin_%s-%s_r%skm_fold%s.npy' %(c.model_type, self.train_type,
                                                                                                                    self.hazard, self.wofs_spinup_time,
                                                                                                                    self.forecast_length, start_min,
                                                                                                                    end_min, self.radius, fold)
            else:
                all_srs_fname = '%s_%s_trained_all_rf_%s_sr_probs_spinup%smin_length%smin_%s-%s_r%skm_fold%s.npy' %(c.model_type, self.train_type,
                                                                                                                    self.hazard, self.wofs_spinup_time,
                                                                                                                    self.forecast_length, start_min,
                                                                                                                    end_min, self.radius, fold)
            all_probs_fname = '%s_%s_trained_all_rf_%s_raw_probs_spinup%smin_length%smin_%s-%s_r%skm_fold%s.npy' %(c.model_type, self.train_type,
                                                                                                                   self.hazard, 
                                                                                                                   self.wofs_spinup_time,
                                                                                                                   self.forecast_length, start_min,
                                                                                                                   end_min, self.radius, fold)
            all_events_fname = '%s_%s_trained_all_%s_events_spinup%smin_length%smin_%s-%s_r%skm_fold%s.npy' %(c.model_type, self.train_type, self.hazard,
                                                                                                           self.wofs_spinup_time,
                                                                                                           self.forecast_length,
                                                                                                           start_min, end_min, self.radius, fold)
        
        return save_dir, all_srs_fname, all_probs_fname, all_events_fname
    
    def get_test_probs_fnames_dir(self, fold, init_date_str, init_time_str, start_time_str, end_time_str):
        save_dir = '%s/%s/wofslag_%s/length_%s/%s/%s' %(self.test_dir, self.hazard, self.wofs_spinup_time, self.forecast_length, init_date_str,
                                                        init_time_str)
        if self.train_type == 'warnings':
            if c.use_avg_sr_map:
                save_file_sr = '%s_%s_trained_rf_%s_avg_sr_probs_%s_init%s_v%s-%s.txt' %(c.model_type, self.train_type, self.hazard, init_date_str, 
                                                                                     init_time_str, start_time_str, end_time_str)
            else:
                save_file_sr = '%s_%s_trained_rf_%s_sr_probs_%s_init%s_v%s-%s.txt' %(c.model_type, self.train_type, self.hazard, init_date_str, 
                                                                                     init_time_str, start_time_str, end_time_str)
            save_file_raw = '%s_%s_trained_rf_%s_raw_probs_%s_init%s_v%s-%s.txt' %(c.model_type, self.train_type, self.hazard, init_date_str,
                                                                                   init_time_str, start_time_str, end_time_str)
            
            events_file = '%s_%s_trained_rf_%s_events_%s_init%s_v%s-%s.txt' %(c.model_type, self.train_type, self.hazard, init_date_str,
                                                                                   init_time_str, start_time_str, end_time_str)
            
        else:
            if c.use_avg_sr_map:
                save_file_sr = '%s_%s_trained_rf_%s_avg_sr_probs_%s_init%s_v%s-%s_r%skm.txt' %(c.model_type, self.train_type, self.hazard, init_date_str,
                                                                                           init_time_str, start_time_str, end_time_str, self.radius)
            else:
                save_file_sr = '%s_%s_trained_rf_%s_sr_probs_%s_init%s_v%s-%s_r%skm.txt' %(c.model_type, self.train_type, self.hazard, init_date_str,
                                                                                           init_time_str, start_time_str, end_time_str, self.radius)
            save_file_raw = '%s_%s_trained_rf_%s_raw_probs_%s_init%s_v%s-%s_r%skm.txt' %(c.model_type, self.train_type, self.hazard, init_date_str,
                                                                                         init_time_str, start_time_str, end_time_str, self.radius)
            events_file = '%s_%s_trained_rf_%s_events_%s_init%s_v%s-%s_r%skm.txt' %(c.model_type, self.train_type, self.hazard, init_date_str,
                                                                                         init_time_str, start_time_str, end_time_str, self.radius)
        
        return save_dir, save_file_sr, save_file_raw, events_file
    
    def get_plot_fnames_dir(self, init_date_str, init_time_str, start_time_str, end_time_str):
        save_dir = '%s/%s/wofslag_%s/length_%s/%s/%s'\
        %(self.test_dir, self.hazard, self.wofs_spinup_time, self.forecast_length, init_date_str,
                                                        init_time_str)
        if self.train_type == 'warnings':
            if c.use_avg_sr_map:
                plot_file_sr = '%s_%s_trained_rf_%s_avg_sr_probs_%s_init%s_v%s-%s.png'\
                %(c.model_type, self.train_type, self.hazard, init_date_str, init_time_str,\
                  start_time_str, end_time_str)
            else:
                plot_file_sr = '%s_%s_trained_rf_%s_sr_probs_%s_init%s_v%s-%s.png'\
                %(c.model_type, self.train_type, self.hazard, init_date_str, init_time_str,\
                  start_time_str, end_time_str)
            plot_file_raw = '%s_%s_trained_rf_%s_raw_probs_%s_init%s_v%s-%s.png' %(c.model_type, self.train_type, self.hazard, init_date_str,
                                                                                   init_time_str, start_time_str, end_time_str)
        else:
            if c.use_avg_sr_map:
                plot_file_sr = '%s_%s_trained_rf_%s_avg_sr_probs_%s_init%s_v%s-%s_r%skm.png'\
                %(c.model_type, self.train_type, self.hazard, init_date_str, init_time_str,\
                  start_time_str, end_time_str, self.radius)
            else:
                plot_file_sr = '%s_%s_trained_rf_%s_sr_probs_%s_init%s_v%s-%s_r%skm.png'\
                %(c.model_type, self.train_type, self.hazard, init_date_str, init_time_str,\
                  start_time_str, end_time_str, self.radius)
            plot_file_raw = '%s_%s_trained_rf_%s_raw_probs_%s_init%s_v%s-%s_r%skm.png'\
            %(c.model_type, self.train_type, self.hazard, init_date_str, init_time_str,\
              start_time_str, end_time_str, self.radius)
        
        return save_dir, plot_file_sr, plot_file_raw
    
    def get_plot_titles(self, init_date_str, init_time_str, start_time_str, end_time_str):
        if self.train_type == 'warnings':
            if c.use_avg_sr_map:
                title_sr = '%s %s Trained %s Mapped Average SRs\n%s init %s valid %s-%s' %(c.model_type, self.train_type.capitalize(), self.hazard.capitalize(),
                                                                                            init_date_str, init_time_str, start_time_str, end_time_str)
            else:
                title_sr = '%s %s Trained %s Mapped SRs\n%s init %s valid %s-%s' %(c.model_type, self.train_type.capitalize(), self.hazard.capitalize(),
                                                                                            init_date_str, init_time_str, start_time_str, end_time_str)
            title_raw = '%s %s Trained %s Raw Probs\n%s init %s valid %s-%s' %(c.model_type, self.train_type.capitalize(), self.hazard.capitalize(),
                                                                                        init_date_str, init_time_str, start_time_str, end_time_str)
        else:
            if c.use_avg_sr_map:
                title_sr = '%s %s Trained %s Mapped Average SRs\n%s init %s valid %s-%s %skm Radius' %(c.model_type, self.train_type.capitalize(),
                                                                                               self.hazard.capitalize(), init_date_str, init_time_str,
                                                                                               start_time_str, end_time_str, self.radius)
            else:
                title_sr = '%s %s Trained %s Mapped SRs\n%s init %s valid %s-%s %skm Radius' %(c.model_type, self.train_type.capitalize(),
                                                                                               self.hazard.capitalize(), init_date_str, init_time_str,
        ...
        return title_sr, title_raw
        
    
    def get_sr_map_fname_dir(self, fold):
        start_min = self.lead_time
        end_min = self.lead_time + self.forecast_length
        sr_map_dir = '%s/%s/wofslag_%s/length_%s/all_raw_probs_fold%s' %(self.validation_dir, self.hazard, self.wofs_spinup_time,
                                                                                 self.forecast_length, fold)
        if self.train_type == 'warnings':
            sr_map_fname = '%s_%s_trained_sr_map_%s_spinup%smin_length%smin_%s-%s_fold%s.csv' %(c.model_type, self.train_type, self.hazard,
                                                                                                self.wofs_spinup_time, self.forecast_length,
                                                                                                start_min, end_min, fold)
        else:
            sr_map_fname = '%s_%s_trained_sr_map_%s_spinup%smin_length%smin_%s-%s_r%skm_fold%s.csv' %(c.model_type, self.train_type, self.hazard,
                                                                                                      self.wofs_spinup_time, self.forecast_length,
                                                                                                      start_min, end_min, self.radius, fold)
        
        return sr_map_dir, sr_map_fname
    
    def get_avg_sr_map_fname_dir(self):
        start_min = self.lead_time
        end_min = self.lead_time + self.forecast_length
        sr_map_dir = '%s/%s/wofslag_%s/length_%s' %(self.validation_dir, self.hazard, self.wofs_spinup_time,
                                                                                 self.forecast_length)
        if self.train_type == 'warnings':
            sr_map_fname = '%s_%s_trained_avg_sr_map_%s_spinup%smin_length%smin_%s-%s.csv' %(c.model_type, self.train_type, self.hazard,
                                                                                                self.wofs_spinup_time, self.forecast_length,
                                                                                                start_min, end_min)
        else:
            sr_map_fname = '%s_%s_trained_avg_sr_map_%s_spinup%smin_length%smin_%s-%s_r%skm.csv' %(c.model_type, self.train_type, self.hazard,
                                                                                                      self.wofs_spinup_time, self.forecast_length,
                                                                                                      start_min, end_min, self.radius)
        
        return sr_map_dir, sr_map_fname
        
    def get_probs_events(self, fold):
        save_dir = '%s/%s/wofslag_%s/length_%s/all_raw_probs_fold%s' %(self.validation_dir, self.hazard, self.wofs_spinup_time, self.forecast_length, fold)
        events_fname = '%s_%s_trained_all_%s_events_spinup%smin_length%s_min%s-%s_r%skm_fold%s.npy' %(c.model_type, self.train_type, self.hazard, self.wofs_spinup_time,\
                                                                                                      self.forecast_length, self.lead_time, self.lead_time+self.forecast_length,\
                                                                                                      self.radius, fold)
        raw_probs_fname = '%s_%s_trained_all_rf_%s_raw_probs_spinup%smin_length%s_min%s-%s_r%skm_fold%s.npy' %(c.model_type, self.train_type, self.hazard, self.wofs_spinup_time,\
                                                                                                         self.forecast_length, self.lead_time,\
                                                                                                         self.lead_time+self.forecast_length, self.radius, fold)
        events = np.load('%s/%s' %(save_dir, events_fname))
        raw_probs = np.load('%s/%s' %(save_dir, raw_probs_fname))
        return raw_probs, events
    ...
    def get_srs(f, x, thresholds, include_less_10 = False):
        ...
        for i in range(len(thresholds)):
            ...
            pos_indices = np.where(f >= t)
            f_positive = pos_indices[0].size
            pos_x = x[pos_indices]
            ...
            if include_less_10:
                if len(srs) == 0:
                    ...
                    sr_max = max(srs)
                if (tp + fp) >= c.success_ratio_threshold:
                    to_append = max(sr_max, SR, t)
                else:
                    to_append = max(sr_max, t)
    ...
    def probs_to_srs(probs, sr_map_df):
        '''takes in one dimensional probability field and converts it to 1d and 2d SR field'''
        ...
        for i in range(len(probs)):
            srs1d[i] = sr_dict[str(probs[i])]
        srs2d = srs1d.reshape(MLTrainer.GRID_SHAPE_X, MLTrainer.GRID_SHAPE_Y)
        
        return srs1d, srs2d
    
    @staticmethod
    def plot_probs(probs, obs, obs_color, levels, init_date, title, save_dir, save_file, cmap = 'Blues', obs_lw = 0.25):
    ...
    @staticmethod
    def get_avg_sr_fname_dir(haz, train_type, lead, r, length):
        validation_dir = '/work/ryan.martz/wofs_phi_data/%s_train/validation_fcsts/%s' %(train_type, c.model_type)
        sr_dir = '%s/%s/wofslag_%s/length_%s' %(validation_dir, haz, c.wofs_spinup_time, length)
        sr_fname = '%s_%s_trained_avg_sr_map_%s_spinup%smin_length%smin_%s-%s_r%skm.csv' %(c.model_type, train_type, haz, c.wofs_spinup_time,
                                                                                           length, lead, lead+length, r)
        return sr_dir, sr_fname
    
    @staticmethod
    def postprocess_sr_map_by_lead_time(train_type, length):
        for r in c.train_radii:
            for haz in c.train_hazards:
                for i in range(1, len(c.train_lead_times)):

                    lead_curr = c.train_lead_times[i]
                    if lead_curr + length > 240:
                        continue
                    sr_dir_curr, sr_fname_curr = MLTrainer.get_avg_sr_fname_dir(haz, train_type, lead_curr, r, length)
                    ...
                    sr_dir_prev, sr_fname_prev = MLTrainer.get_avg_sr_fname_dir(haz, train_type, lead_prev, r, length)
    ...
    @staticmethod
    def postprocess_sr_map_by_obs_limiting(length):
        for r in c.train_radii:
            for haz in c.train_hazards:
                for lead in c.train_lead_times:
                    ...
                    obs_sr_dir, obs_sr_fname = MLTrainer.get_avg_sr_fname_dir(haz, 'obs', lead, r, length)
                    obs_sr_map = pd.read_csv('%s/%s' %(obs_sr_dir, obs_sr_fname))
                    obs_srs = np.array(obs_sr_map.SR)
                    obs_probs = np.array(obs_sr_map.raw_prob)

                    obsw_sr_dir, obsw_sr_fname = MLTrainer.get_avg_sr_fname_dir(haz, 'obs_and_warnings', lead, r, length)
...
class PS:
    ...
    HISTORICAL_VARIABLES = ["id", "hail_prob", "torn_prob", "wind_prob", "age", "east_motion", "south_motion"]
    ...
    @classmethod
    def preprocess_ps(cls, grid, specs, ps_path, ps_files, wofs_obj):
        ''' Like the "main method"/blueprint method for doing the probSevere preprocessing.
            ...
            @wofs_obj is the object with all of the wofs predictor data. Also stores background
                u and v fields created from pressure weighted ensemble means of u and v wind at
                10m and 850, 700, and 500 hPa. This background flow is used as a first guess for
                PS motion when PS objects are early in their lifetimes
        ...
        extrapolated_gdf = PS.do_extrapolation(past_ps_df, merged_gdf, specs, cls.ALL_EXTRAP_DF_KEYS, wofs_obj, grid)
    ...
    @staticmethod
    def do_extrapolation(prev_ps_df, curr_gdf, fcst_specs, df_column_names, wofs_obj, grid):
        ''' This method creates/returns a "final" geopandas dataframe with all relevant PS 
            ...
            @wofs_obj is the object that stores wofs predictor data and the first guess
                storm motion fields based on pressure weighted ensemble means of wofs
                wind up to 500 hPa
            @grid is a grid object based on the current wofs grid
        '''
        
        #### TODO: ensure wofs_obj is actually inputted ####
        ...
        if (len(curr_gdf) == 0):
            ...
            for obj_id in obj_ids: 
                ...
                obj_geometry = PS.get_object_attribute(obj_gdf, "geometry")

                #Find Storm Age
                obj_age = PS.find_obj_age(obj_id, prev_ps_df)
                
                #If object is less than 10 minutes old (less than 6 objects in the list),
                #then we test to see if the PS motion is "close enough" to the background
                #first guess motion. If not, replace PS motion with first guess field
                #value. Note that PS motion is always used for 10+ minute old objects
                if obj_age <= 10:
                    #get centroid of current PS object
                    centroid_lat = float(obj_geometry.centroid.y)
                    centroid_lon = float(obj_geometry.centroid.x)

                    #find closest wofs grid point to PS object centroid
                    dists = utilities.haversine(grid.lats, grid.lons, centroid_lat, centroid_lon)
                    grid_i = np.where(dists == np.min(np.abs(dists)))[0][0]
                    grid_j = np.where(dists == np.min(np.abs(dists)))[1][0]
                    
                    first_guess_east = wofs_obj.first_guess_u[grid_i, grid_j]
                    first_guess_south = wofs_obj.first_guess_v[grid_i, grid_j] * (-1)
                    
                    # if more than 10 m/s off, then use first guess field
                    #if (np.sqrt(((obj_motion_east - first_guess_east)**2) + ((obj_motion_south - first_guess_south)**2)) > 0.6):
                    #    obj_motion_east = first_guess_east
                    #    obj_motion_south = first_guess_south
    ...
    def get_past_ps_df(cls, specs, ps_path, ps_files, current_ids):
        ...
        for p in range(len(ps_files)):
            ps_file = ps_files[p]
            ps_data = PS.get_ps_data(ps_path, ps_files[p])
            object_exists = (ps_data != "")
            
            age = specs.ps_ages[p]

            if object_exists:  #i.e., if there's at least one storm object
    ...
    def extract_ps_info(cls, ps_data, age, ps_version, isCurrent): 
        ...
        if (isCurrent == False): 
            df = pd.DataFrame(list(zip(ids, hail_probs, torn_probs, wind_probs, ages, east_motion, south_motion)),\
                              columns=cls.HISTORICAL_VARIABLES)
...
class TORP:
    def __init__(self, ID, prob, lat, lon, last_update_str, torp_df_row, fcst_specs, wofs_spinup):
        ...
        self.fcst_specs = fcst_specs
        self.set_start_valid()
        #self.start_valid = fcst_specs.ps_init_time_dt + dt.timedelta(minutes = fcst_specs.ps_lead_time_start)
    ...
    def __gt__(self, other):
        ...
        if self.detection_time > other.detection_time:
            return True
        elif (self.detection_time == other.detection_time) and (self.last_update > other.last_update):
            return True
        else:
            return False
    ...
    def set_start_valid(self):
        '''Unused for now, using ps init time instead of creating my own logic. Leaving this function here for potential
        future use. Moving to ps init time since the start valid time here doesn't account for 5 minute update logic'''
        
        if (self.fcst_specs.ps_init_time_dt.minute % 2 == 0):
            test_start = self.fcst_specs.ps_init_time_dt + dt.timedelta(minutes = 1)
        else:
            test_start = self.fcst_specs.ps_init_time_dt
        
        
        count = 0
        exit = False
        while (exit == False) and (count < 6):
            if ((test_start - self.last_update).seconds/60 < 30) and ((test_start - self.last_update).seconds >= 0):
                self.start_valid = test_start
                exit = True
                return
            else:
                test_start -= dt.timedelta(minutes = 30)
                count += 1
        
        if count >= 6 and exit == False:
            self.start_valid = test_start
        
        #self.start_valid = self.fcst
        #curr_hour = self.last_update.hour
        #curr_min = self.last_update.minute
        #curr_sec = self.last_update.second
        #if ((curr_min < (c.wofs_bottom_init_min + self.wofs_spinup_time)) or (curr_min == (c.wofs_bottom_init_min + self.wofs_spinup_time) and curr_sec == 0)) and (curr_min >= self.wofs_spinup_time):
        #    print(c.wofs_spinup_time)
        #    print(c.bottom_init_min)
        #    self.start_valid = self.last_update.replace(minute = c.wofs_bottom_init_min + self.wofs_spinup_time, second = 0)
        #else:
        #    if curr_min >= (c.wofs_bottom_init_min + self.wofs_spinup_time):
        #        self.start_valid = (self.last_update + dt.timedelta(hours = 1))
        #        self.start_valid = self.start_valid.replace(minute = self.wofs_spinup_time, second = 0)
        #    else:
        #        self.start_valid = self.last_update.replace(minute = self.wofs_spinup_time, second = 0)
    ...
    def update_buffers(self):
        ...
        line = LineString([self.Points[1], self.Points[2]])
        
        local_azimuthal_projection = "+proj=aeqd +R=6371000 +units=m +lat_0={} +lon_0={}".format((self.lats[1] + self.lats[2]) / 2, (self.lons[1] + self.lons[2]) / 2)
    ...
    def get_wofs_overlap_points(self, wofs_gdf, torp_dict, *args):
        ...
        torp_gdf = gpd.GeoDataFrame(index=[0], crs='epsg:4326', geometry=[self.geometrys[1]])
    ...
    @staticmethod
    def get_torp_files(wofs_file, training = False):
        ...
        earliest_possible = most_recent_possible - dt.timedelta(minutes = c.torp_file_search_minutes)
        
        if training:
            start_dir = '%s/%s' %(c.raw_torp_training_dir, dt.datetime.strftime(earliest_possible, '%Y%m%d'))
            if os.path.exists(start_dir):
                for torp_csv in os.listdir(start_dir):
                    if not ('.csv' in torp_csv):
                        continue
            ...
            end_dir = '%s/%s' %(c.raw_torp_training_dir, dt.datetime.strftime(most_recent_possible, '%Y%m%d'))
            ...
            if earliest_possible.hour < 12:
                date_dir = earliest_possible - dt.timedelta(days = 1)
            else:
                date_dir = earliest_possible
            torp_dir = '%s/%s' %(c.torp_dir, dt.datetime.strftime(date_dir, '%Y%m%d'))
            if os.path.exists(torp_dir):
                for radar in os.listdir(torp_dir):
                    radar_dir = '%s/%s/netcdf/torp/TORPcsvShort/QC' %(torp_dir, radar)
                    for torp_csv in os.listdir(radar_dir):
                        if not ('.csv' in torp_csv):
                            continue
                        date_time = utilities.parse_date(torp_csv.split('_')[0])
                        if (date_time >= earliest_possible) and (date_time <= most_recent_possible):
                            torp_files.append('%s/%s' %(radar_dir, torp_csv))
...
class TORP_List:
    ...
    def gen_full_dict_from_file_list(paths, grid, fcst_specs):
        ...
        for i, path in enumerate(paths):
            if i == 0:
                torp_dict = TORP_List.gen_torp_dict_from_file(path, grid, fcst_specs, c.wofs_spinup_time)
            else:
                torp_dict = TORP_List.gen_torp_dict_from_file(path, grid, fcst_specs, c.wofs_spinup_time, td = torp_dict)
        
        return torp_dict
    
    @staticmethod
    def gen_wofs_points_gdf(torp_dict, fcst_specs, wofs_gdf, lead_time):
        ...
        if (fcst_specs.ps_init_time_dt.minute % 2 == 0):
            test_start = fcst_specs.ps_init_time_dt + dt.timedelta(minutes = 1)
        else:
            test_start = fcst_specs.ps_init_time_dt
        ...
        for long_id in torp_dict:
            l = torp_dict[long_id]
            t = l.array[0]
            if not (t.start_valid == test_start):
                continue
            if i == 0:
    ...
    @staticmethod
    def overlap_gdf_to_npy(gdf, torp_dict, txt_file, wofs_grid, fcst_specs):
        ...
        num_x_points = wofs_grid.nx
        num_y_points = wofs_grid.ny
        total_grid_points = num_x_points * num_y_points
        ...
        for predictor in predictors:
            npy_predictors_dict[predictor] = np.zeros((num_x_points, num_y_points))
            if predictor in ['torp_age']:
                npy_predictors_dict[predictor] -= 1
            elif predictor in c.torp_min_convs:
                npy_predictors_dict[predictor] += 9999
        ...
        for m in range(len(wofs_i)):
            ...
            for predictor in npy_predictors_dict:
                ...
                if predictor == 'extrapolation_minutes':
                    pred = TORP_List.get_extrapolation(gdf, torp_dict, m, fcst_specs)
                else:
        ...
        for i, predictor in enumerate(predictors):
            ...
            if not (var_method == "none"):
                ...
                array_1d_15km = array_2d_15km.reshape((total_grid_points,1))
                array_2d_30km = utilities.add_convolutions(var_method, array_2d, conv_footprints[1])
                array_1d_30km = array_2d_30km.reshape((total_grid_points,1))
                array_2d_45km = utilities.add_convolutions(var_method, array_2d, conv_footprints[2])
                array_1d_45km = array_2d_45km.reshape((total_grid_points,1))
                array_2d_60km = utilities.add_convolutions(var_method, array_2d, conv_footprints[3])
                array_1d_60km = array_2d_60km.reshape((total_grid_points,1))
            
            array_1d = array_2d.reshape((total_grid_points,1))
            if i == 0:
                full_npy = array_1d
                if not (var_method == "none"):
    ...
    @staticmethod
    def get_extrapolation(gdf, torp_dict, i, fcst_specs):
        t_id = gdf.iloc[i].torp_id
        t_list = torp_dict[t_id]
        t = t_list.array[0]
        t_cen_lat = t.lats[1]
        t_cen_lon = t.lons[1]
        
        test_point = gdf.iloc[i].geometry
        test_lat = test_point.y
        test_lon = test_point.x
        
        #convert speeds from m/s to km/min
        t_speed_north = (t.storm_motion_north/1000)*60
        t_speed_east = (t.storm_motion_east/1000)*60
        t_speed = math.sqrt(t_speed_north**2 + t_speed_east**2)
        
        start_extrapolation = t.time_to_start_valid + t.lead_time - t.wofs_spinup_time
        extrapolation = start_extrapolation
        test_radius = copy.deepcopy(c.torp_point_buffer)
        dist = utilities.haversine(t_cen_lat, t_cen_lon, test_lat, test_lon)
        ...
        while extrapolation <= (fcst_specs.forecast_window + start_extrapolation):
            
            prev_t_cen_lon = t_cen_lon
            t_cen_lon = utilities.haversine_get_lon(t_cen_lat, prev_t_cen_lon, t_speed_east)
            avg_lon = (prev_t_cen_lon + t_cen_lon)/2
            t_cen_lat = utilities.haversine_get_lat(t_cen_lat, avg_lon, t_speed_north)
    ...
    def gen_torp_npy(torp_files, wofs_grid, fcst_specs, lead_time):
        ...
        if len(torp_files) == 0:
            ...
            npy = TORP_List.overlap_gdf_to_npy(gdf, td, c.torp_vars_filename, wofs_grid, fcst_specs)
        
        return npy
    
def train(dates, length, lead_time, haz, train_type):
    
    if length + lead_time > 240:
        return
    
    if (length == 120) and (train_type == 'obs'):
        return
    ...
    trainer = MLTrainer(dates, length, lead_time, c.num_folds, haz, c.wofs_spinup_time, radius, train_type, buffer_str)
    trainer.do_training()
    
    return

def test(dates, length, lead_time, haz, train_type):
...
def main():
    ...
    if c.train_mode == "train" or c.train_mode == "validate" or c.train_mode == "make_maps":
        iterator = md.to_iterator(dates, c.forecast_lengths, c.train_lead_times, c.train_hazards,\
                                  c.train_types)
        results = md.run_parallel(train, iterator, nprocs_to_use = 15,\
        ...
        #    for length in [60]:#c.forecast_lengths:
    ...
    #results = md.run_parallel(test, iterator, nprocs_to_use = 18,\
    ...
    #                                continue
    #                            print(length)
    #                            trainer = MLTrainer(dates, length, lead_time, c.num_folds, haz, c.wofs_spinup_time, radius, train_type)
    #                            trainer.do_training()
    #        for length in c.forecast_lengths:
    #            MLTrainer.postprocess_sr_map_by_lead_time(train_type, length)
    #    if 'obs' in c.train_types:
