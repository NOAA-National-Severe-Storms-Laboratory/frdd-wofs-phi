...
def plot_wofs_phi_forecast_mode(nc_fname, png_outdir, wofs_init_dt, before_00z_date,\
    ...
    for t in range(len(training_types)):
        ...
        for h in c.final_hazards:
            for r in c.final_str_obs_radii:
                vars_to_plot.append('wofsphi__%s__%skm__%smin__%s' \
                        %(h, r, time_window, training_type))
    ...
    for v in data.keys():
        ...
        png_save_name = "%s_%s_%s_%s_f%s.png" %(v, before_00z_date, wofs_init_time_str,\
...
def plot_wofs_phi_warning_mode(nc_fname, png_outdir, wofs_init_dt, \
    ...
    for v in data.keys():
        ...
        if ("tornado" in v):
            cont_kwargs_dict = {'levels': orig_levels, 'extend': extend_var, 'linewidths': orig_linewidths_tornado, \
                        'linestyles': 'solid', 'colors': colors,\
                        'alpha': alpha, 'add_labels': True}

            #Mask out low or zero probabilities
            color_cont_data[color_cont_data<orig_levels[0]] = -1.0 
