...
class Warning_Grid():
    ...
    def get_full_1d_warnings_filename(haz, start, length):
        directory = '%s/%s/%s' %(c.train_warnings_full_1d_npy_dir, length, haz)
    ...
    def merge_obs_warnings(haz, start, length, r):
        ...
        events_1d = np.max(warnings, obs)
        events_2d = np.reshape(events_1d, (MLTrainer.GRID_SHAPE_X, MLTrainer.GRID_SHAPE_Y))
        
        save_dir_1d = '%s/%s/%s' %(c.train_obs_and_warnings_full_1d_npy_dir, length, haz)
        save_1d_file = '%s_obs_and_warnings_%s_v%s-%s_r%skm_1d.npy' %(haz, start_day_str, start_time_str, end_time_str, r)
        
        save_dir_2d = '%s/%s/%s' %(c.train_obs_and_warnings_full_2d_npy_dir, length, haz)
        save_2d_file = '%s_obs_and_warnings_%s_v%s-%s_r%skm_2d.npy' %(haz, start_day_str, start_time_str, end_time_str, r)
    ...
    def set_raw_gdf(self):
        ...
        for i in range(len(simplified_gdf.WIND_MAP)):
            if combined_gdf.TORNADOTAG.iat[i] == 'RADAR INDICATED' or combined_gdf.TORNADOTAG.iat[i] == 'OBSERVED':
                ...
                simplified_gdf.TORN_MAP.iat[i] = 0.5
