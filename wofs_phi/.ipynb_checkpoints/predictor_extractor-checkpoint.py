...
def save_predictors(full_pred_array, samp_rate, fGrid, full_npy_outdir, npy_filename, \
    ...
    if (samp_rate < 1.):
        ...
        #Save to file:
        #if not os.path.isfile("%s/%s" %(full_npy_outdir, npy_filename)):
        np.save("%s/%s" %(full_npy_outdir, npy_filename), full_predictions)
        #print("%s/%s" %(full_npy_outdir, npy_filename))
        #if not os.path.isfile("%s/%s" %(dat_outdir, dat_filename)):
        sampled_predictions.tofile("%s/%s" %(dat_outdir, dat_filename))
        
        #Save rand_inds too
        #if not os.path.isfile("%s/%s" %(dat_outdir, rand_inds_filename)):
