...
def haversine(lat1, lon1, lat2, lon2):
    ...
    first = np.power((np.sin(dLat)), 2)
    second = np.power(1 - np.sin(dLat), 2)
    third = np.power(np.sin(avgLat), 2)
    fourth = np.power(np.sin(dLon), 2)
    
    dist = 2*r*np.arcsin(np.sqrt(first + ((second - third) * fourth)))
